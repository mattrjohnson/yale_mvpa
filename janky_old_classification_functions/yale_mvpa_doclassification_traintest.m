...
results(n_trainsets,n_testsets).train_eegfile_trial_inds =  cell( classifier.nits, 1 );
results(n_trainsets,n_testsets).test_eegfile_trial_inds =   cell( classifier.nits, 1 );
...
for i=1:n_trainsets
    ...
    sub.trainset(i).n_trials_per_cond = ones(sub.trainset(i).n_conds,1) * size(sub.trainset(i).classifier_data{1},2);
    sub.testset(i).n_trials_per_cond  = ones(sub.testset(i).n_conds,1)  * size(sub.testset(i).classifier_data{1},2);
...
%         train_eegfile_trial_inds_tmp =  zeros( size(traintargs,2), 2 );
...
%commented out for Img_Perc fMRI
%             train_eegfile_trial_inds_tmp(logical(traintargs(k,:)),1) = ...
%                                         sub.trainset(i).eegfile_inds{k}(inds);
%             train_eegfile_trial_inds_tmp(logical(traintargs(k,:)),2) = ...
%                                         sub.trainset(i).trial_inds{k}(inds);
        end
        ...
        for k=1:n_testsets
            ...
            %commented out for Img_Perc fMRI
%             test_eegfile_trial_inds_tmp =                   zeros( n_testtrials_total, 2 );
...
%commented out for Img_Perc fMRI
%                 test_eegfile_trial_inds_tmp(testpat_start_ind:testpat_end_ind,1) = ...
%                                         sub.testset(k).eegfile_inds{m}(:);
%                 test_eegfile_trial_inds_tmp(testpat_start_ind:testpat_end_ind,2) = ...
%                                         sub.testset(k).trial_inds{m}(:);
            ...
            %commented out for Img_Perc fMRI
%             results(i,k).train_eegfile_trial_inds{j} =      train_eegfile_trial_inds_tmp;
%             results(i,k).test_eegfile_trial_inds{j} =       test_eegfile_trial_inds_tmp;
        end
    end
end
